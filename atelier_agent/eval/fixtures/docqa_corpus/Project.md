...
- 2026-08-10 — Temporary local model roles: brain=`qwen3:8b`, worker=`hf.co/LiquidAI/LFM2.5-2.6B-GGUF:Q6_K`, heavy=`gemma4:26b`; qwen3:14b remains a future candidate because it is not installed.
...
- 2026-07-06 — Do **not** ship the planner-router adapter into the live routing path yet — measured route accuracy is only 40% (2/5) on the held-out set; the difficulty router (binary, 256 examples, 100%) stays the production router. Rationale: the planner-router's data is the bottleneck (32 train / 5 test), not the method; format compliance was fully learned but the semantic routing decision was not. Next data build should expand to hundreds of rows and close the `category` label set before re-training.
...
- 2026-07-06 — Trained + evaluated the planner-router adapter (the open Phase-6 follow-up). LoRA on Qwen2.5-0.5B-Instruct-4bit, 200 iters (~100s, val loss 2.503→0.338), adapter at `models/router/planner_adapter/`. Added `models/router/evaluate_planner.py` (structured-JSON scorer: parsed-JSON rate, per-field acc, exact-match, route acc — base vs fine-tuned on the 5-task held-out set) and wired it into `make train-planner-router`. **Result (honest):** base model 0% parseable JSON; fine-tuned 100% parseable and 100% correct `tool_plan`, but exact-match only 20% and `model_route` acc only 40% — it over-routes cheap tasks to `brain` and invents open-vocab `category` values. Conclusion: the fine-tune reliably taught *output format*, not the *routing decision*; the bottleneck is dataset size (32 train / 5 test), not the method. Difficulty router remains the production router; planner-router is not promoted into the live path (see Decision Log 2026-07-06).

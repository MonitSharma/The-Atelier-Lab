...
def test_validate_reports_locked_protocol_without_executing(capsys):
    exit_code = main(["validate"])

    captured = capsys.readouterr()
    assert exit_code == 0
    assert "execution_ready: true" in captured.out
    assert "structurally_valid: true" in captured.out
...
def test_smoke_with_locked_protocol_config_runs_toy_only(tmp_path: Path, capsys):
    output = tmp_path / "smoke.json"

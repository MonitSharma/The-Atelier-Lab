...
- **Name:** Atelier
- **Status:** All phases (0–7) shipped. Dual-mode local agent + memory + MCP + hybrid retrieval + eval harness + a LoRA-fine-tuned router + one-command repro + writeup. Eval suite expanded to 18 doc-QA + 13 code-repair tasks + 3 combined knowledge→build tasks, with SVG plot generation and recorded expanded results. Current live results: doc-QA 94% correct / 83% retrieval / 100% cited, code 100%, combined 100%. Open follow-ups: fine-tune planner-router, broaden combined eval, public release.
- **Owner:** Monit
...
- 2026-06-22 — Added combined knowledge→build eval suite (`eval/tasks_combined/`) with 3 tasks. Combined success requires both passing tests and `search_notes` in the trace. Added planner-router SFT dataset builder (`models/router/make_planner_dataset.py`) producing 34 rows from real eval metadata: task → category, difficulty, edit scope, tool plan, model route.
- 2026-06-22 — Live combined eval passed: 3/3 solved, 3/3 tests passed, 3/3 used `search_notes`, average 6 steps (`report_20260621T174453`). Combined plots copied into `docs/assets/eval/report_20260621T174453/`.
